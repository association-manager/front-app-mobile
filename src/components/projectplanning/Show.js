...
import { retrieve, reset } from '../../actions/projectplanning/show';
import { del, loading, error } from '../../actions/projectplanning/delete';
import { list } from '../../actions/projectplanning/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.projectplanningCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.projectplanningUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
  ...
  render() {
    ...
    return (
        <View style={ {flex: 1} }>
          <ScrollView>
            {item &&
            <Card title="ProjectPlanning">
              <List title="title">
                {Show.renderRow('id', item['@id'])}
                {Show.renderRow('name', item['name'])}
                {Show.renderRow('startAt', item['startAt'])}
                {Show.renderRow('endAt', item['endAt'])}
              </List>
            </Card>
            }
...
const mapStateToProps = (state) => {
  return {
    error: state.projectplanning.show.error,
    loading: state.projectplanning.show.loading,
    retrieved: state.projectplanning.show.retrieved,
    deleteError: state.projectplanning.del.error,
    deleteLoading: state.projectplanning.del.loading,
    deleted: state.projectplanning.del.deleted,
  };
};
