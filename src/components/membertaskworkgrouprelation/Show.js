...
import { retrieve, reset } from '../../actions/membertaskworkgrouprelation/show';
import { del, loading, error } from '../../actions/membertaskworkgrouprelation/delete';
import { list } from '../../actions/membertaskworkgrouprelation/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.membertaskworkgrouprelationCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.membertaskworkgrouprelationUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
  ...
  render() {
    ...
    return (
        <View style={ {flex: 1} }>
          <ScrollView>
            {item &&
            <Card title="MemberTaskWorkGroupRelation">
              <List title="title">
...
const mapStateToProps = (state) => {
  return {
    error: state.membertaskworkgrouprelation.show.error,
    loading: state.membertaskworkgrouprelation.show.loading,
    retrieved: state.membertaskworkgrouprelation.show.retrieved,
    deleteError: state.membertaskworkgrouprelation.del.error,
    deleteLoading: state.membertaskworkgrouprelation.del.loading,
