...
export function mercureOpen(eventSource) {
  return { type: 'TASK_UPDATE_MERCURE_OPEN', eventSource };
}

export function mercureMessage(retrieved) {
  return dispatch => {
    if (1 === Object.keys(retrieved).length) {
      dispatch({ type: 'TASK_UPDATE_MERCURE_DELETED', retrieved });
      return;
    }

    dispatch({ type: 'TASK_UPDATE_MERCURE_MESSAGE', retrieved });
  };
}
