...
By default, Association Manager starts like a regular React Native application. In case you want it to run
...
In Association Manager, we handle it with [`.expo` file extension](https://docs.expo.io/versions/latest/bare/using-expo-client/#practical-patterns-for-client-compatible-bare-apps).

# Environments

Association Manager application supports two environments: 
...
- Clone Eva Design System and UI Kitten projects to the directory containing Association Manager:
```bash
...
We use [Typescript](https://www.typescriptlang.org/) to build Association Manager.
...
All screens presented in Association Manager application are listed in [`./src/layouts`](./src/layouts) directory and structured by following categories: 
